...
class Logs(commands.Cog):
    ...
    async def on_message_delete(self, message):
        ...
        embed.add_field(name = "Deleted by", value = f'{deleter.mention} ({deleter.id})')
        embed.add_field(name = "Deleter is a staff ?", value = "Mod" if (deleter.guild_permissions.value & (1 << 3)) == 1 << 3 else "User")
